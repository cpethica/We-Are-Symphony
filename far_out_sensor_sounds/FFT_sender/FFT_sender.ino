...
void loop()
{
  ...
  {
    vReal[i] = int8_t(analogRead(sensorPin));/* Build data with positive and negative values*/
...
//  PrintVector(vImag, samples, SCL_INDEX);
  ...
  // calculate and send peak frequency to 6dp
  double x = FFT.MajorPeak(vReal, samples, samplingFrequency);

    Serial.println(x, 6);
    // sendOSC1("/peak", x);        /// send peak frequency data
  //while(1); /* Run Once */
...
void PrintVector(double *vData, uint16_t bufferSize, uint8_t scaleType)
{
  for (uint16_t i = 0; i < bufferSize; i++)
  {
    ...
    sendOSC("/freq", abscissa, vData[i]);     /// send fft data
  }
  Serial.println();
}

void sendOSC(String msg, double data1, double data2) {
...
void sendOSC1(String msg, double data1) {
